use dart_re_analyzer::mcp::{McpServer, McpRequest, ErrorsQuery, DiagnosticsStats};
...
async fn test_mcp_get_all_errors_request() {
    ...
    let diag1 = create_test_diagnostic("test1.dart", Severity::Error, RuleCategory::Runtime, "rule1");
    let diag2 = create_test_diagnostic("test2.dart", Severity::Warning, RuleCategory::Style, "rule2");
...
async fn test_mcp_get_errors_filtered_by_category() {
    ...
    let diag1 = create_test_diagnostic("test1.dart", Severity::Error, RuleCategory::Runtime, "rule1");
    let diag2 = create_test_diagnostic("test2.dart", Severity::Warning, RuleCategory::Style, "rule2");
    let diag3 = create_test_diagnostic("test3.dart", Severity::Error, RuleCategory::Runtime, "rule3");
    ...
    let diagnostics: Vec<Diagnostic> = serde_json::from_value(response.data).unwrap();
    assert_eq!(diagnostics.len(), 2);
    assert!(diagnostics.iter().all(|d| matches!(d.category, RuleCategory::Runtime)));
}
...
async fn test_mcp_get_errors_filtered_by_severity() {
    ...
    let diagnostics: Vec<Diagnostic> = serde_json::from_value(response.data).unwrap();
    assert_eq!(diagnostics.len(), 1);
    assert!(diagnostics.iter().all(|d| matches!(d.severity, Severity::Warning)));
}
...
async fn test_mcp_get_errors_filtered_by_file() {
    ...
    let diag1 = create_test_diagnostic("lib/main.dart", Severity::Error, RuleCategory::Runtime, "rule1");
    let diag2 = create_test_diagnostic("lib/utils.dart", Severity::Warning, RuleCategory::Style, "rule2");
    let diag3 = create_test_diagnostic("lib/main.dart", Severity::Error, RuleCategory::Runtime, "rule3");
    ...
    let diagnostics: Vec<Diagnostic> = serde_json::from_value(response.data).unwrap();
    assert_eq!(diagnostics.len(), 2);
    assert!(diagnostics.iter().all(|d| d.location.file.contains("main.dart")));
}
...
async fn test_mcp_get_errors_multiple_filters() {
    ...
    let diag1 = create_test_diagnostic("lib/main.dart", Severity::Error, RuleCategory::Runtime, "rule1");
    let diag2 = create_test_diagnostic("lib/main.dart", Severity::Warning, RuleCategory::Runtime, "rule2");
    let diag3 = create_test_diagnostic("lib/utils.dart", Severity::Error, RuleCategory::Runtime, "rule3");
    let diag4 = create_test_diagnostic("lib/main.dart", Severity::Error, RuleCategory::Style, "rule4");
    
    mcp.update_diagnostics(vec![diag1, diag2, diag3, diag4]).await;
...
async fn test_mcp_get_stats() {
    ...
    let diag4 = create_test_diagnostic("test3.dart", Severity::Error, RuleCategory::Runtime, "rule4");
...
async fn test_filtered_diagnostics_query() {
    ...
    let diag1 = create_test_diagnostic("test.dart", Severity::Error, RuleCategory::Runtime, "rule1");
    let diag2 = create_test_diagnostic("test.dart", Severity::Warning, RuleCategory::Style, "rule2");
